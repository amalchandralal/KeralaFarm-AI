...
// Fetch directly from Nominatim in the browser
async function fetchFromNominatim(lat: number, lon: number, radius: number): Promise<Place[]> {
  const keywords = ['Krishi Bhavan', 'Agriculture Office', 'Krishi Vigyan Kendra']
  const seen = new Set<string>()
  const results: Place[] = []
...
function FlyTo({ lat, lon }: { lat: number; lon: number }) {
  const map = useMap()
  useEffect(() => { map.flyTo([lat, lon], 15, { duration: 1.2 }) }, [lat, lon, map])
  return null
}

function RecenterMap({ center }: { center: [number, number] }) {
  const map = useMap()
  useEffect(() => { map.flyTo(center, 11, { duration: 1.5 }) }, [center, map])
  return null
}
    ...
    <div className="pt-24 pb-20 page-container">
      <div className="px-2 mb-8">
        <h1 className="flex items-center gap-3 text-3xl font-black text-slate-900">
          <span className="p-2 bg-emerald-100 rounded-2xl">📍</span>
          Agricultural Places
        </h1>
        <p className="mt-2 text-sm font-black tracking-widest uppercase text-emerald-600" style={{ fontFamily: 'Noto Sans Malayalam, sans-serif' }}>കൃഷി കേന്ദ്രങ്ങൾ</p>
        <p className="mt-2 font-medium text-slate-500">Find Krishi Bhavans, farm support centers, and more across Kerala</p>
      </div>

      {outsideKerala && (
        <div className="flex items-center gap-3 p-4 mx-2 mb-6 text-sm font-bold text-blue-700 border border-blue-100 bg-blue-50 rounded-2xl">
           <span className="text-xl">ℹ️</span>
           You're outside Kerala — showing nearest agricultural offices in your area.
        </div>
      )}

      <div className="flex flex-col gap-4 px-2 mb-4 sm:flex-row">
        <div className="relative flex-1">
          <input
            type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search places by name…" 
            className="w-full px-5 py-4 text-sm font-bold transition-all bg-white border outline-none border-slate-200 rounded-2xl focus:ring-4 focus:ring-emerald-500/10"
          />
        </div>
        <button onClick={locateMe} disabled={locating}
          className="flex items-center justify-center gap-3 px-8 py-4 font-black text-white transition-all shadow-lg bg-emerald-600 hover:bg-emerald-700 rounded-2xl shadow-emerald-200 active:scale-95 disabled:opacity-50 whitespace-nowrap">
          {locating
            ? <><div className="w-5 h-5 border-white rounded-full border-3 border-t-transparent animate-spin" /> Locating…</>
            : ' Near Me'}
        </button>
      </div>

      <div className="flex flex-col gap-3 px-2 mb-6 sm:flex-row">
        <input
          type="text" value={citySearch} onChange={e => setCitySearch(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && searchByCity()}
          placeholder="Or search by city / district name…"
          className="flex-1 px-5 py-3 text-xs font-bold transition-all border outline-none bg-slate-50 border-slate-100 rounded-2xl focus:ring-4 focus:ring-emerald-500/10"
        />
        <button onClick={searchByCity} disabled={cityLoading || !citySearch.trim()}
          className="flex items-center justify-center gap-2 px-6 py-3 text-xs font-black transition-all text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-2xl disabled:opacity-50 whitespace-nowrap">
          {cityLoading
            ? <><div className="w-3 h-3 border-2 rounded-full border-emerald-600 border-t-transparent animate-spin" /> Searching…</>
            : ' Search City'}
        </button>
      </div>

      <div className="flex items-center gap-4 px-4 mb-8 overflow-x-auto scrollbar-hide">
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Radius:</span>
        {[10000, 20000, 50000].map(r => (
          <button key={r} onClick={() => { setRadius(r); const pos = userPos ?? [10.8505, 76.2711]; fetchPlaces(pos[0], pos[1], r) }}
            className={`px-5 py-2 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${radius === r ? 'bg-emerald-600 text-white border-emerald-600 shadow-md' : 'bg-white border-slate-200 text-slate-500 hover:border-emerald-400 hover:text-emerald-600'}`}>
            {r / 1000} km
          </button>
        ))}
      </div>
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 mx-2 mb-6 text-sm font-bold text-red-700 border border-red-100 bg-red-50 rounded-2xl">
          <span className="text-xl">⚠️</span> {error}
        </div>
      )}

      <div className="mx-2 mb-8 overflow-hidden border-4 border-white shadow-2xl rounded-[2.5rem] relative" style={{ height: '450px' }}>
        <MapContainer center={mapCenter} zoom={10} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
          <TileLayer
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            ...
            <Marker key={p.id} position={[p.lat, p.lon]} icon={greenIcon}
              ...
              <Popup>
                <div className="p-1">
                  ...
                  <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">{typeLabel[p.type] || ' Agri Center'}</p>
                  {p.address  && <p className="mt-2 text-xs font-medium leading-relaxed text-slate-500">{p.address}</p>}
                  {p.distance != null && <p className="mt-2 font-black text-blue-600 text-[10px] uppercase tracking-wider"> {p.distance.toFixed(1)} km away</p>}
                </div>
              </Popup>
            </Marker>
          ))}
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center gap-4 py-12">
          ...
          <p className="text-xs font-black tracking-widest uppercase text-emerald-700">Finding agricultural centers…</p>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div className="flex items-center justify-between px-4 mb-6">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            Found <span className="text-emerald-600">{filtered.length}</span> results
            {userPos && ' · sorted by proximity'}
          </p>
        </div>
      )}
      ...
      {!loading && !error && places.length === 0 && (
        <div className="px-4 py-20 text-center">
          <div className="flex items-center justify-center w-20 h-20 mx-auto mb-6 text-4xl bg-slate-50 rounded-3xl">🔍</div>
          <p className="text-xl font-black text-slate-900">No places found</p>
          <p className="mt-1 text-sm font-black text-emerald-600" style={{ fontFamily: 'Noto Sans Malayalam, sans-serif' }}>സ്ഥലങ്ങൾ കണ്ടെത്തിയില്ല</p>
          <p className="mt-4 text-sm font-medium text-slate-500">Try "Near Me", search a city, or increase the radius</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 px-2 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map(p => (
          <div key={p.id} onClick={() => setSelected(p)}
            className={`bg-white border rounded-[2rem] p-6 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 ${selected?.id === p.id ? 'border-emerald-500 ring-4 ring-emerald-500/10' : 'border-slate-100'}`}>
            <div className="flex items-start gap-4">
              <div className="flex items-center justify-center w-12 h-12 text-2xl bg-emerald-50 rounded-2xl shrink-0">🌿</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black truncate text-slate-900">{p.name}</p>
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">{typeLabel[p.type] || '🌱 Agri Center'}</p>
                {p.address && <p className="mt-2 text-xs font-medium truncate text-slate-400"> {p.address}</p>}
                {p.phone   && <a href={`tel:${p.phone}`} onClick={e => e.stopPropagation()}
                  className="inline-block mt-2 text-xs font-black text-emerald-600 hover:underline"> {p.phone}</a>}
                {p.distance != null && (
                  <div className="mt-3 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse" />
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest"> {p.distance.toFixed(1)} km away</p>
                  </div>
                )}
              </div>
            </div>
            <button onClick={e => { e.stopPropagation(); setSelected(p) }}
              className="mt-5 w-full text-[10px] font-black uppercase tracking-widest text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl py-3 transition-all">
              View on map →
            </button>
          </div>
        ))}
